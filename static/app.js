(() => {
  ...
  function renderEmail(data) {
    ...
    const showTab = tab => { content.innerHTML = emailTabContent(tab, data); animateGauges(content); content.querySelectorAll('[data-copy]').forEach(btn => btn.onclick = async () => { await navigator.clipboard?.writeText(btn.dataset.copy); btn.textContent = 'Copied'; setTimeout(() => { btn.textContent = btn.dataset.copy; }, 900); }); if (tab === 'report') $('#download-report').onclick = async () => { const button = $('#download-report'); const label = button.textContent; button.disabled = true; button.textContent = 'Building report…'; try { const response = await fetch('/api/export-report', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({report_id:data.report_id})}); if (!response.ok) { const error = await response.json().catch(() => ({})); throw new Error(error.error || 'Report export failed.'); } const blob = await response.blob(), link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'scamshield-forensic-report.html'; link.click(); URL.revokeObjectURL(link.href); } catch (error) { $('#download-status').textContent = error.message; } finally { button.disabled = false; button.textContent = label; } }; };
