(() => {
  ...
  function renderUrl(data) {
    ...
    const dnsList = ['spf', 'dkim', 'dmarc'].map(key => { const exists = dns[key]?.exists; return `<div class="flex items-center justify-between rounded-lg bg-slate-900 p-3"><span class="uppercase text-slate-400">${key}</span><span class="font-medium">${exists ? '<span class="text-green-400">Present</span>' : badge(key === 'dmarc' ? 'Medium' : 'Low')}</span></div>`; }).join('');
    ...
    result.innerHTML = `${verdictCard}<div class="mb-5 grid gap-5 lg:grid-cols-2">${section('Domain information', `<dl class="space-y-4 text-sm">${domainInfoList}<div class="flex justify-between"><dt class="text-slate-400">Domain age risk</dt><dd>${domainAgeBadge}</dd></div></dl>`)}${section('Published DNS protections', `<div class="space-y-3">${dnsList}</div>`)}</div><div>${section('Suspicious indicators', indicatorTags)}</div>`;
